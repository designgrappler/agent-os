...
- **Real-Time Discovery & Roster Building**: 
    ...
    3. **SPECIALIST INTERVIEW**: Ask the user: *"How many specialized roles do we need for the Implementation Team, and which skill should each role use?"*
...
- **DNA Mapping**: Update `.agent/context/AGENTIC.md` with the formal Org Chart, including the specific `Skill_ID` for every assigned persona.
- **Implementation Team Taxonomy**: Explicitly use the term **"Implementation Team"** to describe the collective Tier 3 personnel in all documentation.
...
2. **Registry Check**: Inspect `.agent/context/AGENTIC.md`. Ensure every person is mapped to a `Skill_ID`.
