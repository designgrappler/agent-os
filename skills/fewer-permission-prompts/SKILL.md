...
4. **Drop commands Claude Code already auto-allows.** These don't need an allowlist entry — they never prompt. If you see any of these in the transcripts, skip them.
...
## When to Run
Run after a few sessions on a new project, or whenever you notice repeated permission prompts for the same commands.

## Stats
- **Overhead:** Low (one-time scan)
- **Requires:** Claude Code with transcript access (`~/.claude/projects/`)
- **Benefit:** Eliminates recurring prompts for safe, read-only operations
