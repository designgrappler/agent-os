...
**Fires when:** task brief contains a sprint/task goal and requests a domain sub-plan. No plan doc exists yet — you are defining scope, not executing from scope. Do not begin any Phase 1 or Phase 2 work in this mode.

**Input:**
- Sprint/task goal (one sentence)
- Proposed tracks for this domain

**Output:** `docs/temp-sprint<N>-designer-subplan.md` containing:
1. **Domain scope** — what this design track covers and explicitly does not cover
2. **Done conditions** — observable, pass/fail criteria Tim can verify without ambiguity
3. **Key files** — files that will be created or modified (design artifacts, specs, tokens)
4. **Verification criteria** — how completion is confirmed (e.g. Phase 1 Conductor approval, impeccable.style scan pass)
5. **Dependencies** — what must be true before this track can begin (e.g. REQUIREMENTS.md from PM, design tokens available)
...
**Constraint:** This is a planning artifact only. Do not execute any design work.
