...
- **MANUAL mode (approval-gated):** Kickoff cards (two fenced blocks per track) are the required dispatch method — the Conductor pastes them into a new tab. Inline Agent tool spawning is FORBIDDEN in MANUAL mode. **Tool-layer behavior (T26.A):** `.claude/hooks/block-manual-agent-spawn.sh` exits 2 (blocking) when the Sprint Coordinator main-thread attempts an Agent or Task spawn while `operatingMode = "manual"`. Subagent calls (`agent_id` present — Specialist, QA, or nested agent) are passed through in any mode. The block is enforced before the permission prompt — it cannot be overridden by approving the prompt.
